...
### DESCRIPTION:
  ...
  #   additional 'addCircleMarkers' sections; for example, copy line 199-219 
  #   and edit the 'filter' arguments (first line) and the 'group' text 
  #   (last line) then go to 'overlayGroups' (starting line 467) and add an 
  #   entry for the group you just created. You can of course remove checkboxes
  #   by doing the opposite (remove the addCircleMarkers section and the 
  #   corresponding overlayGroups entry).
...
for(i in 1:length(taxa_cycle)){
  ...
  try(final_map <- leaflet() %>%
    ...
    addCircleMarkers(data = taxon_now %>% filter(!.yrna & database!="Ex_situ"),
      ...
      # .unc (records that have an uncertainty greater than your max, or no uncertainty)
      addCircleMarkers(data = taxon_now %>% filter(!.unc & database!="Ex_situ"),
                       ~decimalLongitude, ~decimalLatitude,
                       popup = ~paste0(
                         "<b>Accepted species name:</b> ",taxon_name_accepted,"<br/>",
                         "<b>Verbatim taxon name:</b> ",taxon_name,"<br/>",
                       ...
                       radius=4, stroke=T, color="black", weight=1,
                       fillColor="red", fillOpacity=0.8,
                       group = "No coordinate uncertainty or > designated max (.unc)") %>%
      # .elev (records that are outside the taxon's elevation range)
      addCircleMarkers(data = taxon_now %>% filter(!.elev & database!="Ex_situ"),
                       ~decimalLongitude, ~decimalLatitude,
                       popup = ~paste0(
                         "<b>Accepted species name:</b> ",taxon_name_accepted,"<br/>",
                         "<b>Verbatim taxon name:</b> ",taxon_name,"<br/>",
                       ...
                       radius=4, stroke=T, color="black", weight=1,
                       fillColor="red", fillOpacity=0.8,
                       group = "Elevation outside taxon's range (.elev)") %>%
      # .rec (records that are not current and/or native based on basisOfRecord and establishmentMeans)
      addCircleMarkers(data = taxon_now %>% filter(!.rec & database!="Ex_situ"),
                       ~decimalLongitude, ~decimalLatitude,
                       popup = ~paste0(
                         "<b>Accepted species name:</b> ",taxon_name_accepted,"<br/>",
                         "<b>Verbatim taxon name:</b> ",taxon_name,"<br/>",
                       ...
                       radius=4, stroke=T, color="black", weight=1,
                       fillColor="red", fillOpacity=0.8,
                       group = "Fossil, cultivated, introduced, or managed (.rec)") %>%
    ...
    addLayersControl(
      ...
      overlayGroups = c("Within 500m of country/state centroid (.cen)",
                        ...
                        "No coordinate uncertainty or > designated max (.unc)",
                        "Elevation outside taxon's range (.elev)",
                        "Fossil, cultivated, introduced, or managed (.rec)"
                        ),
    ...
    hideGroup("No coordinate uncertainty or > designated max (.unc)") %>%
    ## Legend
